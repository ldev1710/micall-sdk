...
void (*accStatusListenerPtr)(bool,int) = 0;
...
void MyAccount::onRegState(OnRegStateParam &prm)
{
    ...
    accStatusListenerPtr(ai.regExpiresSec != 0,prm.code);
}

void MyAccount::onIncomingCall(OnIncomingCallParam &iprm)
{
    call = new MyCall(*this,iprm.callId);
    callerId = call->getInfo().remoteUri;
    incomingCallPtr();
}
...
void MyCall::onCallState(OnCallStateParam &prm)
{
    ...
    if(ci.state == PJSIP_INV_STATE_EARLY){
        callStatusListenerPtr(2);
    }
    
    if(ci.state == PJSIP_INV_STATE_CONNECTING){
        callStatusListenerPtr(3);
    }
...
void MyCall::onCallMediaState(OnCallMediaStateParam &prm)
{
    ...
    for (unsigned i = 0; i < ci.media.size(); i++) {
        if (ci.media[i].type==PJMEDIA_TYPE_AUDIO) {
            ...
            callStatusListenerPtr(4);
        }
        if(ci.media[i].status == PJSUA_CALL_MEDIA_LOCAL_HOLD){
            isHolding = true;
            callStatusListenerPtr(7);
        }
        if (ci.media[i].status == PJSUA_CALL_MEDIA_ACTIVE){
            if(isHolding){
                callStatusListenerPtr(8);
                isHolding = false;
            }
        }
    }
...
void PJSua2::createAccount(std::string username, std::string password,
                           std::string registrar, std::string port,std::string sipProxy)
{
    // Configure an AccountConfig
    AccountConfig acfg;
    acfg.idUri = "sip:" + username + "@" + registrar + ":" + port;
    acfg.sipConfig.proxies.clear();
    acfg.sipConfig.proxies.push_back(sipProxy);
    acfg.regConfig.registrarUri = "sip:" + registrar;
...
void PJSua2::transfer(std::string dest_uri)
{
...
void PJSua2::toggleMute(bool isMute)
{
    ...
    for (unsigned i = 0; i < ci.media.size(); i++) {
        if (ci.media[i].type==PJMEDIA_TYPE_AUDIO) {
            ...
            AudDevManager& mgr = Endpoint::instance().audDevManager();
            aud_med->startTransmit(mgr.getPlaybackDevMedia());
            if(isMute) mgr.getCaptureDevMedia().stopTransmit(*aud_med);
            else mgr.getCaptureDevMedia().startTransmit(*aud_med);
        }
    }
    callStatusListenerPtr(isMute ? 5 : 6);
}
 ...
 */
void PJSua2::acc_listener(void (* funcpntr)(bool,int))
{
